...
#define getSuperblock(fs) ((Superblock *) (fs))
#define initSuperblock Superblock *superblock = getSuperblock(fs)
#define getPointerByIndex(data, byteSize, number) \
 ((uint8_t *) (data) + (byteSize) * ((number) - 1))
#define getPointerByDataNumber(superblock, number) \
 ((Block *) getPointerByIndex((Block *) (superblock) + (superblock)->start_data, BS,  (number)))
#define getPointerByINodeNumber(superblock, number) \
 ((INode *) getPointerByIndex((Block *) (superblock) + (superblock)->first_inode, INODE_SIZE, (number)))
#pragma pack(push)
#pragma pack(1)
...
#define OBJ_NAME_LENGTH (BS-8-4-1)
#define FS_CATALOG (1)
#define FS_FILE (2)
